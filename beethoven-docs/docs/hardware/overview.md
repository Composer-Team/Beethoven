...
Second, there is the [accelerator configuration](/docs/hardware/overview/#configuration--build).
...
between other cores on-chip [here](/docs/hardware/cross-core)
...
memory compiler support [here](/docs/hardware/asic-memory-compiler).
...
- `latency`: Like scratchpads, user-managed memory also takes a latency parameter. For FPGAs, the use of this
    is straightforward. For ASICs, we cascade memory cells to a similar effect (see [here](/docs/hardware/asic-memory-compiler)).
...
associated `Address` type [here](/docs/software/overview/#beethoven-runtime).
...
- [Zynq Ultrascale+ Series](https://www.amd.com/en/products/adaptive-socs-and-fpgas/soc/zynq-ultrascale-plus-mpsoc.html) -
    ...
    for our development purposes. [See here](/docs/platforms/kria) for a walkthrough on deploying a design on Kria.
...
If you're interested in developing your own platform, [read here](/docs/platforms/custom-platform).
