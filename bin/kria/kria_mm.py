...
current_alloc = int(open(f"/sys/kernel/mm/hugepages/{idx}/nr_hugepages", 'r').read().strip())
current_free = int(open(f"/sys/kernel/mm/hugepages/{idx}/free_hugepages", 'r').read().strip())

print(f"Current allocation of [{idx}]: {current_free}/{current_alloc} available")
...
with open(f"/sys/kernel/mm/hugepages/{idx}/nr_hugepages", 'w') as f:
    f.write(str(set_to))
print(f"Set allocation of [{idx}] to {set_to}. Waiting a second to recheck...")
os.system("sleep 1")
print(f"Rechecking...")
current_alloc = int(open(f"/sys/kernel/mm/hugepages/{idx}/nr_hugepages", 'r').read().strip())
if current_alloc != set_to:
    print(f"Error: Failed to set allocation of [{idx}] to {set_to}")
    sys.exit(1)
print(f"Successfully set allocation of [{idx}] to {set_to}")
sys.exit(0)
