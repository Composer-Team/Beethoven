...
# f = open(f"{os.environ.get('COMPOSER_AWS_SDK_DIR')}/hdk/common/shell_v04261818/new_cl_template/design/cl_template.sv")
# g = open(f"{os.environ.get('COMPOSER_ROOT')}/Composer-Hardware/vsim/generated-src/aws_shell.sv")
    ...
    if state == 0:
        ...
        if spl[0] == 'module':
            if spl[1][:-1] == 'ComposerTop':
                state = 1
    elif state == 1: # start scraping ios
        if ln[:2] == ');':
            state = 2
            break
        spl = ln.split()
        if len(spl) == 2: # wire is width 1
            width = 1
            name = spl[1]
        else:
            ...
            for pr in ct_io:
                if pr['name'] != 'clock' and pr['name'] != 'reset':
                    if pr['width'] == 1:
                        ...
                        g.write(f"wire [{pr['width']-1}:0] {pr['wire']};\n")
            for pr in ct_io:
